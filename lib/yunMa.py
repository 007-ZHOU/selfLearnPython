...
class YdmVerify(object):
    ...
    _token = "8CRMRYyaGZGcLbBkYLje-lI9chCjFN2lPaANhVkMGqo"
    _headers = {
    ...
    def slide_verify(self, slide_image, background_image, verify_type="20101"):
        ...
        # 通用双图滑块  20111
        payload = {
            "slide_image": base64.b64encode(open(slide_image, "rb").read()).decode(),
            "background_image": base64.b64encode(open(background_image, "rb").read()).decode(),
            "token": self._token,
            "type": verify_type
        }
    ...
    def rotate(self, out_ring_image, inner_circle_image):
        ...
        payload = {
            "out_ring_image": base64.b64encode(open(out_ring_image, "rb").read()).decode(),
            "inner_circle_image": base64.b64encode(open(inner_circle_image, "rb").read()).decode(),
            "token": self._token,
            "type": "90004"
        }
        resp = requests.post(self._custom_url, headers=self._headers, data=json.dumps(payload))
...
if __name__ == '__main__':
    ...
    image='验证码.jpg'
    verify_type='10110'
    result=YdmVerify().common_verify(image=image,verify_type=verify_type)
    print(result)
